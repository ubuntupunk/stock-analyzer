...
class WatchlistManager {
  ...
  async loadWatchlist() {
    ...
    if (!window.authManager) {
      console.warn(
        'AuthManager not available, cannot check authentication status for watchlist.'
      );
      return;
    }
  ...
  async _doLoadWatchlist() {
    try {
      ...
      console.log(
        '>>> _doLoadWatchlist: container=' + !!container + ', hasContent=' + hasContent
      );
      ...
      if (container && !hasContent) {
        ...
        console.log(
          '>>> _doLoadWatchlist: Already rendered, updating buttons and reloading prices'
        );
        ...
        console.log(
          '>>> _doLoadWatchlist: Section not loaded after wait, rendering anyway'
        );
        // Ensure Add Stock button handler is set up
        this.setupAddStockButtonHandler();
        await this.renderWatchlist();
  ...
  async waitForWatchlistSection() {
    ...
    console.log(
      '>>> waitForWatchlistSection: Section exists:',
      exists,
      'after',
      attempts * 50,
      'ms'
  ...
  async toggleWatchlist(symbol) {
    ...
    console.log(
      'WatchlistManager: Current watchlist before toggle:',
      this.watchlist.map((i) => i.symbol)
    );
    ...
    console.log(
      'WatchlistManager: Watchlist after toggle:',
      this.watchlist.map((i) => i.symbol)
    );
  }
                ...
                <div class="watchlist-content">
                    ...
                    ${
                      item.alertPrice
                        ...
                        <div class="watchlist-alert">
                            <i class="fas fa-bell"></i> Alert: ${this.formatCurrency(
                              item.alertPrice
                            )}
                        </div>
                    `
                    ...
                    <div class="watchlist-meta">
                        <span class="added-date">
                            <i class="fas fa-calendar-plus"></i> Added: ${this.formatDate(
                              item.addedAt
                            )}
                        </span>
                    </div>
  ...
  updateStockCardWatchlistButtons(symbol) {
    ...
    console.log(
      `WatchlistManager: Found ${stockCards.length} stock cards for ${symbol}`
    );
  ...
  async loadWatchlistPrices() {
    console.log(
      '>>> loadWatchlistPrices() CALLED, _pricesLoading=' + this._pricesLoading
    );

    if (!window.authManager) {
      console.warn(
        'AuthManager not available, cannot check authentication status for watchlist prices.'
      );
      return;
    }
  ...
  async _doLoadWatchlistPrices() {
    ...
    try {
      ...
      console.log(
        '>>> _doLoadWatchlistPrices: Batch prices received:',
        Object.keys(batchPrices)
      );
      ...
      if (updated === 0) {
        console.log(
          '>>> _doLoadWatchlistPrices: No prices loaded from API, showing demo'
        );
        this.showDemoPrices();
      }
    } catch (error) {
  ...
  showDemoPrices() {
    const demoPrices = {
      AAPL: { price: 178.5, change: 2.35, change_percent: 1.33 },
      MSFT: { price: 378.25, change: -1.5, change_percent: -0.4 },
      GOOGL: { price: 141.8, change: 3.2, change_percent: 2.31 },
      AMZN: { price: 178.75, change: 1.85, change_percent: 1.05 },
      NVDA: { price: 495.22, change: 12.5, change_percent: 2.59 },
      ABB: { price: 52.4, change: 0.35, change_percent: 0.67 },
    };
    ...
    this.watchlist.forEach((item) => {
      ...
      if (demoPrice) {
        ...
        const randomPercent = (
          (parseFloat(randomChange) / parseFloat(randomPrice)) *
          100
        ).toFixed(2);
  ...
  updateWatchlistPriceDisplay(symbol, priceData) {
    try {
      ...
      if (!priceContainer || !price) {
        console.log(
          `>>> updateWatchlistPriceDisplay: No container or price for ${symbol}`
        );
        return;
      }
      ...
      let priceSpan =
        priceContainer.querySelector('.price-loading') ||
      ...
      if (changeElement && change !== undefined && changePercent !== undefined) {
        ...
        const changeHtml = `<span class="${changeClass}">${changeSymbol}${Formatters.formatStockPrice(
          change
        )} (${changeSymbol}${changePercent.toFixed(2)}%)</span>`;

        if (changeSpan) {
          changeSpan.className = changeClass;
          changeSpan.textContent = `${changeSymbol}${Formatters.formatStockPrice(
            change
  ...
  async loadPopularStockTags() {
    ...
    try {
      ...
      container.innerHTML = popularStocks
        .slice(0, 12)
        .map(
          (stock) => `
  ...
  async searchAndDisplayStocks(query, resultsContainer) {
    ...
    try {
      ...
      if (stocks && stocks.length > 0) {
        resultsContainer.innerHTML = stocks
          .map(
            (stock) => `
                    <div class="stock-picker-item" data-symbol="${
                      stock.symbol
                    }" data-name="${stock.name || stock.company_name || stock.symbol}">
                        <div class="stock-info">
                            <span class="stock-symbol">${stock.symbol}</span>
                            <span class="stock-name">${
                              stock.name || stock.company_name || stock.symbol
                            }</span>
                        </div>
                        <button class="btn-add-stock" onclick="window.watchlistManager.addStockFromPicker('${
                          stock.symbol
                        }', '${stock.name || stock.company_name || stock.symbol}')">
        ...
        const isValidSymbol =
          upperQuery.length >= 1 && upperQuery.length <= 5 && /^[A-Z]/.test(upperQuery);
  ...
  async addStockDirectly(symbol) {
    ...
    try {
      ...
      console.log(
        'Could not fetch company name for',
        upperSymbol,
        '- using symbol as name'
      );
    }
  ...
  editWatchlistItem(symbol) {
    ...
    modal.innerHTML = `
            ...
            <div class="watchlist-modal-body">
                ...
                <div class="form-group">
                    <label>Notes</label>
                    <textarea id="editNotes" class="form-control" rows="3" placeholder="Add notes about this stock...">${
                      item.notes || ''
                    }</textarea>
                </div>
                <div class="form-group">
                    <label>Price Alert ($)</label>
                    <input type="number" id="editAlertPrice" class="form-control" value="${
                      item.alertPrice || ''
                    }" step="0.01" placeholder="Alert when price reaches...">
                </div>
            </div>
  ...
  onShow() {
    console.log(
      '[WatchlistManager] Shown - resuming price updates and restoring scroll'
    );
    this.isVisible = true;
    this.startPriceUpdates();
  ...
  saveScrollPosition() {
    ...
    if (grid) {
      ...
      console.log(
        '[WatchlistManager] Scroll position saved:',
  ...
  restoreScrollPosition() {
    ...
    if (grid && this.savedScrollPosition > 0) {
      ...
      console.log(
        '[WatchlistManager] Scroll position restored:',
        this.savedScrollPosition
      );
    }
  }
