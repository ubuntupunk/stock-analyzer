...
class UIManager {
  ...
  updateFinancialsDisplay(financials, symbol) {
    ...
    this.updateElement(
      'financialsUpdated',
      financials.timestamp || new Date().toLocaleString()
    );
  ...
  updateFinancialsTable(tableId, data) {
    ...
    if (!tbody) {
      ...
      console.log(
        'UIManager: Available elements with IDs:',
    ...
    if (!data || data.length === 0) {
      ...
      tbody.innerHTML =
        '<tr><td colspan="5" class="loading">No financial data available</td></tr>';
      console.log(
        `=== UIManager: updateFinancialsTable END (${tableId}) - NO DATA ===`
      );
      return;
    }
    ...
    if (tableElement && years.length > 0) {
      ...
      console.warn(
        `UIManager: Could not find table element or no years for ${tableId}`
      );
    }
    ...
    tbody.innerHTML =
      html || '<tr><td colspan="5" class="loading">No data available</td></tr>';
  ...
  updateStockAnalyserDisplay(data, symbol) {
    if (data.price) {
      this.updateElement(
        'currentPrice',
        Formatters.formatStockPrice(data.price.currentPrice)
      );

      // Calculate price change if historical data available
      if (
        data.price.historicalData &&
        Object.keys(data.price.historicalData).length > 1
      ) {
        ...
        const previousPrice = parseFloat(
          data.price.historicalData[dates[dates.length - 2]]['4. close']
        );
        const priceChange = (
    ...
    const analyserHeader = document.querySelector(
      '#stock-analyser .analyser-header h2'
    );
    if (analyserHeader) {
  ...
  updateFactorsDisplay(factors, symbol) {
    ...
    const basicFactors = [
      {
        name: 'P/E Ratio',
        value: factors.pe_ratio || 'N/A',
        status: this.evaluateFactor(factors.pe_ratio, 'pe'),
      },
      {
        name: 'ROIC',
        value: factors.roic ? Formatters.formatPercentage(factors.roic) : 'N/A',
        status: this.evaluateFactor(factors.roic, 'roic'),
      },
      {
        name: 'Revenue Growth',
        value: factors.revenue_growth
          ? Formatters.formatPercentage(factors.revenue_growth)
          : 'N/A',
        status: this.evaluateFactor(factors.revenue_growth, 'growth'),
      },
      {
        name: 'Debt to Equity',
        value: factors.debt_to_equity || 'N/A',
        status: this.evaluateFactor(factors.debt_to_equity, 'debt'),
      },
      {
        name: 'Current Ratio',
        value: factors.current_ratio || 'N/A',
        status: this.evaluateFactor(factors.current_ratio, 'current'),
      },
      {
        name: 'Price to FCF',
        value: factors.price_to_fcf || 'N/A',
        status: this.evaluateFactor(factors.price_to_fcf, 'fcf'),
      },
    ];
  ...
  updateNewsDisplay(news, symbol) {
    ...
    if (!newsArray || newsArray.length === 0) {
      container.innerHTML =
        '<p class="empty-message">No news available for this stock</p>';
      return;
    }
      ...
      .map(
        (article) => `
            <div class="news-item">
                ...
                <div class="news-time">${Formatters.formatDate(
                  article.publishedAt,
                  'time'
                )}</div>
  ...
  updateEstimatesDisplay(data, symbol) {
    ...
    if (
      !data ||
      (!data.earnings_estimates?.length && !data.revenue_estimates?.length)
    ) {
      container.innerHTML =
        '<p class="empty-message">No analyst estimates available for this stock</p>';
      return;
    }
    ...
    if (data.earnings_estimates?.length > 0) {
      html +=
        '<div class="estimates-section"><h3>Earnings Estimates</h3><table class="estimates-table"><thead><tr><th>Period</th><th>Avg</th><th>Low</th><th>High</th><th>Analysts</th></tr></thead><tbody>';
      data.earnings_estimates.forEach((estimate) => {
        html += `<tr><td>${estimate.period || 'N/A'}</td><td>${Formatters.formatNumber(
          estimate.avg
        )}</td><td>${Formatters.formatNumber(
          estimate.low
        )}</td><td>${Formatters.formatNumber(estimate.high)}</td><td>${
          estimate.num_analysts || 0
        }</td></tr>`;
      });
    ...
    if (data.revenue_estimates?.length > 0) {
      html +=
        '<div class="estimates-section"><h3>Revenue Estimates</h3><table class="estimates-table"><thead><tr><th>Period</th><th>Avg</th><th>Low</th><th>High</th></tr></thead><tbody>';
      data.revenue_estimates.forEach((estimate) => {
        html += `<tr><td>${estimate.period || 'N/A'}</td><td>${Formatters.formatNumber(
          estimate.avg
        )}</td><td>${Formatters.formatNumber(
          estimate.low
        )}</td><td>${Formatters.formatNumber(estimate.high)}</td></tr>`;
      });
      html += '</tbody></table></div>';
    }
  ...
  evaluateFactor(value, type) {
    ...
    switch (type) {
      ...
      case 'roic':
        return value > 0.15 ? 'good' : value > 0.1 ? 'warning' : 'poor';
      case 'growth':
        return value > 0.1 ? 'good' : value > 0.05 ? 'warning' : 'poor';
      case 'debt':
  ...
  updateBreadcrumbs(symbol, searchType) {
    ...
    const breadcrumbSymbolContainer = document.getElementById(
      'breadcrumb-symbol-container'
    );
    ...
    if (breadcrumbTab) {
      if (symbol) {
        ...
        console.log(
          'UIManager: Breadcrumb tab updated to:',
          searchType || 'Popular Stocks'
        );
      }
    }
  ...
  updatePriceIndicators(priceData) {
    this.updateElement(
      'atClosePrice',
      Formatters.formatStockPrice(priceData.currentPrice)
    );

    if (priceData.afterHoursPrice) {
      this.updateElement(
        'afterHoursPrice',
        Formatters.formatStockPrice(priceData.afterHoursPrice)
      );
    }
  }
