...
class RequestQueue {
  ...
  async process() {
    ...
    console.log(
      `RequestQueue: Processing ${item.key || 'anonymous'} (active: ${
    ...
    for (const item of this.queue) {
      try {
        ...
        if (item.attempts >= 3) {
          toRemove.push(item.id);
          console.warn(
            `OfflineQueue: Max attempts reached for ${
              item.request.key || item.request.type
            }`
          );
        }
      }
...
class DataManager {
  ...
  async loadStockData(symbol, type, period = 'annual') {
    ...
    const cacheKey =
      type === 'financials' ? `${symbol}:${type}:${period}` : `${symbol}:${type}`;
    ...
    const task = async () => {
      ...
      try {
        ...
        switch (type) {
          case 'price':
            data = await this.executeWithCircuitBreaker(
            ...
            break;
          case 'metrics': {
            const priceData = await this.executeWithCircuitBreaker(
              () => api.getStockPrice(symbol),
              type
            );
            const metricsData = await this.executeWithCircuitBreaker(
              () => api.getStockMetrics(symbol),
              type
            );
          ...
          case 'financials':
            data = await this.executeWithCircuitBreaker(
              () => api.getFinancialStatements(symbol, period),
              type
            );
          ...
          case 'analyst-estimates':
            data = await this.executeWithCircuitBreaker(
              () => api.getAnalystEstimates(symbol),
              type
            );
  ...
  async searchStocks(query, limit = 10) {
    try {
      ...
      let results;
      try {
        results = await this.executeWithCircuitBreaker(
          () => api.searchStocks(query, limit),
          'search'
        );
      } catch (err) {
  ...
  async getPopularStocks(limit = 12) {
    try {
      ...
      const stocks = await this.executeWithCircuitBreaker(
        () => api.getPopularStocks(limit),
        'popular-stocks'
      );
  ...
  async loadWatchlist() {
    try {
      ...
      const watchlist = await this.executeWithCircuitBreaker(
        () => api.getWatchlist(),
        'watchlist'
      );
      ...
      console.warn(
        'Failed to load watchlist from API, falling back to localStorage:',
        error
      );
  ...
  async addToWatchlist(stockData) {
    try {
      ...
      console.warn(
        'Failed to add to watchlist via API, saving to localStorage:',
        error
      );
  ...
  async removeFromWatchlist(symbol) {
    try {
      ...
      console.warn(
        'Failed to remove from watchlist via API, removing from localStorage:',
        error
      );
