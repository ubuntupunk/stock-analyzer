...
class StockAnalyzer {
  ...
  async initializeModules() {
    ...
    this.modules.watchlistManager = new WatchlistManager(
      this.modules.dataManager,
      eventBus
    );
    ...
    this.modules.stockAnalyserManager = new StockAnalyserManager(
      eventBus,
      api,
      this.modules.dataManager
    );
  ...
  registerModulesWithLifecycle() {
    ...
    if (this.modules.searchManager) {
      lifecycle.registerModule('searchManager', this.modules.searchManager, {
        onInit: () => this.modules.searchManager.onInit?.(),
        onShow: () => this.modules.searchManager.onShow?.(),
        onHide: () => this.modules.searchManager.onHide?.(),
        onDestroy: () => this.modules.searchManager.onDestroy?.(),
      });
    }

    if (this.modules.stockAnalyserManager) {
      lifecycle.registerModule(
        'stockAnalyserManager',
        this.modules.stockAnalyserManager,
        {
          onInit: () => this.modules.stockAnalyserManager.onInit?.(),
          onShow: () => this.modules.stockAnalyserManager.onShow?.(),
          onHide: () => this.modules.stockAnalyserManager.onHide?.(),
          onDestroy: () => this.modules.stockAnalyserManager.onDestroy?.(),
        }
      );
    }
    ...
    if (this.modules.uiManager) {
      lifecycle.registerModule('uiManager', this.modules.uiManager, {
        onInit: () => this.modules.uiManager.onInit?.(),
        onShow: () => this.modules.uiManager.onShow?.(),
        onHide: () => this.modules.uiManager.onHide?.(),
        onDestroy: () => this.modules.uiManager.onDestroy?.(),
      });
    }

    if (this.modules.dataManager) {
      lifecycle.registerModule('dataManager', this.modules.dataManager, {
        onInit: () => this.modules.dataManager.onInit?.(),
        onShow: () => this.modules.dataManager.onShow?.(),
        onHide: () => this.modules.dataManager.onHide?.(),
        onDestroy: () => this.modules.dataManager.onDestroy?.(),
      });
    }

    if (this.modules.healthManager) {
      lifecycle.registerModule('healthManager', this.modules.healthManager, {
        onInit: () => this.modules.healthManager.onInit?.(),
        onShow: () => this.modules.healthManager.onShow?.(),
        onHide: () => this.modules.healthManager.onHide?.(),
        onDestroy: () => this.modules.healthManager.onDestroy?.(),
      });
    }
  ...
  setupModuleCommunication() {
    ...
    eventBus.on('data:loaded', ({ symbol, type, data }) => {
      ...
      const hasHistoricalData =
        (type === 'price' && data?.historicalData) ||
      ...
      if (hasHistoricalData) {
        ...
        if (!canvas) {
          console.log(
            'ChartManager: Canvas not ready yet, chart will be created when tab loads'
          );
          return;
        }

        const chart = this.modules.chartManager.createPriceChart(
          'priceChart',
          data,
          symbol
        );
  ...
  async handleInitialURL() {
    ...
    if (symbol) {
      ...
      const targetTab =
        tab && this.modules.tabManager.isValidTab(tab) ? tab : 'metrics';
  ...
  toggleAuthDropdown() {
    ...
    if (dropdown) {
      ...
      if (isVisible) {
        ...
        console.log(
          'toggleAuthDropdown: this.hCaptchaWidgetId:',
        ...
        if (typeof hcaptcha !== 'undefined' && this.hCaptchaWidgetId === null) {
          ...
          console.log(
            'toggleAuthDropdown: hCaptchaContainer found:',
            !!hCaptchaContainer
          );
          if (hCaptchaContainer) {
            ...
            console.warn(
              'toggleAuthDropdown: hCaptcha container not found in dropdown.'
            );
          }
        } else if (typeof hcaptcha === 'undefined') {
          console.warn(
            'toggleAuthDropdown: hCaptcha object not available, cannot render.'
          );
        } else if (this.hCaptchaWidgetId !== null) {
          console.log(
            'toggleAuthDropdown: hCaptcha already rendered with ID:',
            this.hCaptchaWidgetId
          );
        }
  ...
  updateChartTheme(theme) {
    ...
    if (window.stockManager?.charts) {
      Object.values(window.stockManager.charts).forEach((chart) => {
        if (chart) {
  ...
  async handleQuickSignIn(event) {
    ...
    console.log(
      'handleQuickSignIn: Checking for hCaptcha token and rendering status...'
    );
    ...
    if (!this.hCaptchaToken) {
      if (typeof hcaptcha !== 'undefined' && this.hCaptchaWidgetId !== null) {
        ...
        console.error(
          'handleQuickSignIn: Security check not available. hCaptcha object or widget ID missing.'
        );
        this.showNotification(
          'Security check not available, please try again.',
          'error'
        );
        return;
      }
    ...
    this.showNotification(
      'Proceeding with sign-in using existing security token...',
      'info'
    );
