...
class WatchlistManager:
    ...
    def __init__(self):
        ...
        self.watchlist_table = self.dynamodb.Table(os.environ.get('WATCHLIST_TABLE', 'stock-watchlist'))
    ...
    def add_to_watchlist(self, user_id: str, stock_data: dict) -> dict:
        ...
        try:
            item = {
                ...
                'alertPrice': Decimal(str(stock_data.get('alertPrice'))) if stock_data.get('alertPrice') is not None else None,
...
def lambda_handler(event, context):
    ...
    try:
        if http_method == 'GET' and path == '/api/watchlist':
            ...
            body = json.loads(event.get('body', '{}'))
            result = manager.update_watchlist_item(user_id, body.get('symbol'), body.get('updates', {}))
