...
class StockUniverseManager:
    ...
    def search_stocks(self, query: str, limit: int = 20, region: str = None,
                     index_id: str = None, currency: str = None) -> list:
        """
    ...
    def filter_stocks(self, sector: str = None, min_cap: float = None,
                     max_cap: float = None, market_cap_bucket: str = None,
                     region: str = None, index_id: str = None, currency: str = None) -> list:
        """
        ...
        try:
            ...
            if min_cap is not None or max_cap is not None or market_cap_bucket:
                filtered_items = []
                for item in items:
                    market_cap = float(item.get('marketCapUSD', item.get('marketCap', 0)))
        ...
        try:
            ...
            for index_config in self.index_config.get_all_indices():
                ...
                try:
                    ...
                    response = self.table.scan(
                        FilterExpression=Attr('indexId').eq(index_id),
                        Select='COUNT'
                    )
                    stock_count = response.get('Count', 0)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if '/search' in path:
            ...
            result = manager.filter_stocks(sector, min_cap, max_cap, market_cap_bucket, region, index_id, currency)
