...
class CircuitBreakerManager:
    ...
    Usage:
        ...
        if cb_manager.can_call('yahoo_finance'):
            try:
                ...
                cb_manager.record_failure('yahoo_finance', str(e))
        else:
    ...
    def register_endpoint(self, name: str, config: Optional[CircuitBreakerConfig] = None):
    ...
    def can_call(self, endpoint_name: str) -> bool:
        ...
        with self._lock:
            ...
            if endpoint.state == CircuitState.OPEN:
                ...
                if time.time() - endpoint.last_failure_time > self.config.timeout_seconds:
                    endpoint.state = CircuitState.HALF_OPEN
                    print(f"CircuitBreaker: {endpoint_name} transitioning OPEN → HALF_OPEN")
                    return True
                return False
        ...
        with self._lock:
            ...
            if endpoint.state == CircuitState.HALF_OPEN:
                ...
                if endpoint.successes >= self.config.success_threshold:
                    ...
                    print(f"CircuitBreaker: {endpoint_name} transitioning HALF_OPEN → CLOSED")
            ...
            # In CLOSED state, also check if we should open
                ...
                if endpoint.should_open(self.config.failure_threshold):
                    ...
                    print(f"CircuitBreaker: {endpoint_name} transitioning CLOSED → OPEN")
        ...
        with self._lock:
            if endpoint_name:
                ...
                for name in list(self._endpoints.keys()):
                    self._endpoints[name] = APIEndpoint(name=name)
            print(f"CircuitBreaker: Reset {'all' if endpoint_name is None else endpoint_name}")
    ...
    def get_stats(self) -> Dict:
        ...
        with self._lock:
            total_calls = sum(e.total_calls for e in self._endpoints.values())
            total_errors = sum(e.total_errors for e in self._endpoints.values())
            total_latency = sum(e.total_latency_ms for e in self._endpoints.values())
            open_circuits = sum(1 for e in self._endpoints.values() if e.state == CircuitState.OPEN)
            
            return {
                "total_endpoints": len(self._endpoints),
                "total_calls": total_calls,
                "total_errors": total_errors,
                "overall_success_rate": f"{(1 - total_errors/total_calls * 100) if total_calls > 0 else 100:.1f}%" if total_calls > 0 else "N/A",
                "average_latency_ms": f"{total_latency / total_calls:.1f}" if total_calls > 0 else "N/A",
    ...
    def get_health_report(self) -> Dict:
        ...
        with self._lock:
            ...
            for name, endpoint in self._endpoints.items():
                ...
                elif endpoint.state == CircuitState.HALF_OPEN or endpoint.get_success_rate() < 0.9:
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                cb.record_failure(endpoint_name, type(e).__name__)
                raise
